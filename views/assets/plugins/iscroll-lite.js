...
/**
 * [TagNav å¯ä»¥æ»šåŠ¨çš„æ ‡ç­¾å¯¼èˆª]
 * ä¾èµ–Zeptoã€iScroll
 * @Author: 14100377
 * @param {String} type  è‡ªæ»šåŠ¨ç±»åž‹ scrollToFirst/scrollToNext/simple
 * @param {String} curClassName  å½“å‰é€‰ä¸­çš„ç±»å scrollToFirst/scrollToNext/simple
        ...
        TagNav.prototype = {
            ...
            _init: function(element,opts){
                ...
                if ( self.index === undefined ) {
                    //TODO:ä¼˜å…ˆä»Žé…ç½®è¿˜æ˜¯domé‡Œå–å‘¢
            ...
            _scrollToNextEvent: function(){
                ...
                hub.subscribe('select', function(parms) {
                    // ç¬¬ä¸€è°ƒç”¨çš„æ—¶å€™æ²¡æœ‰prevIndex, å›ºæ ¹æ®this.indexæ¥æŽ§åˆ¶æ–¹å‘ã€‚
                    if ( prevIndex === undefined ) {
                        prevIndex = self.index ? 0 : 1;
                    }
                    var dir = parms.index > prevIndex,
                        // å¦‚æžœæ˜¯æƒ³å·¦åˆ™æ‰¾prevå¦åˆ™æ‰¾next
                        target = self.$items.eq(parms.index)[ dir ? 'next' : 'prev' ](),
                        // å¦‚æžœæ²¡æœ‰ç›¸é‚»çš„ï¼Œè‡ªå·±çš„ä½ç½®ä¹Ÿéœ€è¦æ£€æµ‹ã€‚å­˜åœ¨è¿™ç§æƒ…å†µ
