...
(function (factory) {
    ...
    var calendar = function(e, options) {
        ...
        if (options.scheduleStart && options.scheduleStart.length > 0) {
            for(var x = 0; x < options.scheduleStart.length; x++){
                options.scheduleStart[x] = new Date(options.scheduleStart[x]) || new Date();
            }
        }
        
        if (options.scheduleEnd && options.scheduleEnd.length > 0) {
            for(var x = 0; x < options.scheduleEnd.length; x++){
        ...
        this.classDayScheduled = options.classDayScheduled || 'scheduled';
        ...
        var scheduleEnd = [];
        for(var x = 0; x < options.scheduleStart.length; x++){
            scheduleStart.push(options.scheduleStart[x] || null);
        }
        
        for(var x = 0; x < options.scheduleEnd.length; x++){
            scheduleEnd.push(options.scheduleEnd[x] || null);
        }
    ...
    calendar.prototype.genDate = function(day) {
        ...
        for(var x = 0; x < this.scheduleStart.length; x++){
            if ((this.scheduleStart[x] && writeDay <= this.scheduleEnd[x] && writeDay > this.scheduleStart[x]) ||
                (this.scheduleEnd[x] && this.getYmd(writeDay) == this.getYmd(this.scheduleEnd[x]))) {
                scheduled.push(this.classDayPass + ' ' + this.classDayScheduled);
            }
        }

        var li_classes = [this.classDayBase, old, scheduled, checkDay];
        ...
        $.extend(options, {
            checkDayChange: function(){
                if (this.checkOut == null && this.checkOut == null) {
                    $checkinDate.html('请点击日期选择开始时间');
                    $checkoutDate.html('请点击日期选择结束时间');
                }
                if (this.checkIn ) {
    ...
    CalendarBox.prototype.fillHTML = function(options) {
                        ...
                        <div class="weui_cells_access">\
                            <div class="weui_cell">\
                                <div class="weui_cell_bd weui_cell_primary">\
                                    $start时间：<em id="checkin-date">请点击日期选择开始时间</em>\
                                </div>\
                                $checkin-value\
                            </div>\
                            <div class="weui_cell">\
                                <div class="weui_cell_bd weui_cell_primary">\
                                    $end时间：<em id="checkout-date">请点击日期选择结束时间</em>\
                                </div>\
                                $checkout-value\
                            </div>\
                        </div>\
        ...
        if (options.valueTypes && options.valueTypes.length) {
            html = html.replace(/\$checkin\-value/gi,
                '<div class="weui_cell_ft"><input id="checkin-value" class="weui_input" type="text" value="请点击日期选择开始时间"></div>');
            html = html.replace(/\$checkout\-value/gi,
                '<div class="weui_cell_ft"><input id="checkout-value" class="weui_input" type="text" value="请点击日期选择结束时间"></div>');
        } else {
            html = html.replace(/\$checkin\-value/gi, '');
            html = html.replace(/\$checkout\-value/gi, '');
        }
    ...
    CalendarBox.prototype.bindEvent = function(options) {
        ...
        $btnClose.on(EVENT_TAP, function() {
            ...
            var checkinValue = $checkinValue.val() ? $checkinValue.val().replace('请点击日期选择开始时间', '') : '';
            var checkoutValue = $checkoutValue.val() ? $checkoutValue.val().replace('请点击日期选择结束时间', '') : '';
