    ...
    $('#showFloorPicker').on('click', function (){
        weui.picker([{
            ...
            onConfirm: function (result) {
                ...
                window.location.href = '/v2/leasing?f='+result[0].value+'&type=leasing';
            },
            title: '请选择铺位所在区域'
        });
    });
    ...
    $.each($.parseJSON(sessionStorage.getItem("shops")), function(i,v){
        if((v.unit == "02FL035") || (v.unit == "03FL084") || (v.unit == "03FL001") || (v.unit == "03FL004") || (v.unit == "04FL005") || (v.unit == "04FL008") || (v.unit == "05FL123") || (v.unit == "05FL117") || (v.unit == "07FL069") || (v.unit == "08FL012") || (v.unit == "08FL015")){
            ...
            if(Math.round(area) > minSize && Math.round(area) < maxSize){
                ...
                var fav = '<a href=\'javascript: AddToFavorite("'+buildingCode+'","'+v.code+'","'+storeCode+'","'+v.unit+'");\' style="display: inline-block; font-size: 12px; color: #000; padding: 5px 0; text-align: center; width: 60px;">加入关注</a>';
                if($.inArray(v.code, $.favorites) != -1){
                    fav = '<a href=\'javascript: RemoveFavorite("'+$.favoritesId[$.inArray(v.code, $.favorites)]+'","'+buildingCode+'","'+v.code+'","'+storeCode+'","'+v.unit+'");\' style="display: inline-block; font-size: 12px; color: #000; padding: 5px 0; text-align: center; width: 60px;">取消关注</a>';
                }

                $('.weui-panel__bd').append('<div class="weui-media-box weui-media-box_appmsg" style="background-color: #fff; padding: 15px; margin: 20px;">\n\
...
<span style="position: absolute; right: 16px; font-weight: bold; color: #ddd; background: rgba(0,0,0,0.5); text-align: right; padding: 0 6px; font-size: 12px;">'+v.shopName+'</span>\n\
<div style="padding: 5px 0;">\n\
<a href=\'javascript: showVR("'+vr+'");\' style="display: inline-block; margin: 2px; font-size: 12px; color: #000; padding: 5px 0; text-align: center; width: 60px;">VR</a>\n\
'+fav+'\n\
<a href=\'javascript: showEngineering("'+v.code+'");\' style="display: inline-block; margin: 2px; font-size: 12px; color: #000; padding: 5px 0; text-align: center; width: 60px;">工程条件</a>\n\
<a href=\'javascript: askPrice("'+v.unit+'","'+v.code+'","'+settle_date+'","'+opening_date+'","'+v.shopName+'","'+v.area+'");\' style="display: inline-block; font-size: 12px; color: #000; padding: 5px 0; text-align: center; width: 60px;">申请报价</a>\n\
</div>\n\
</div>\n\
<div style="margin-left: 142px;">\n\
<p class="weui-media-box__desc">面积: '+v.area+'m<sup>2</sup></p>\n\
...
<p class="weui-media-box__desc">推荐业态: '+businessFormatChs+'</p>\n\
<p class="weui-media-box__desc">'+desc+'</p>\n\
    ...
    if(!$.selectedShops.length) {
        $('.weui-panel__bd').html('<div class="weui-media-box weui-media-box_appmsg" style="background-color: #fff; padding: 15px; margin: 20px;">\n\
...
function showEngineering(id){
    $('#engineeringContainer').attr('src','/views/assets/plugins/pdfjs/web/viewer.html?file=/upload/docs/'+id+'.pdf');
