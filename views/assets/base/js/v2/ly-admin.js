var index=0;
var unitCodes = ["1F-37","2F-44B","3F-11","4F-37","4F-38","4F","5F-06","5F-50B","6F-24","6F-50"];
var vr;
...
$(document).ready(function(){
    ...
    for(var j=0;j<=7;j++){
      if(!sessionStorage.getItem('ly_fl_'+j) || sessionStorage.getItem('ly_fl_'+j) == null || sessionStorage.getItem('ly_fl_'+j) == ''){
        getCoordsByFloor(j);
      } else {
        $.each($.parseJSON(sessionStorage.getItem('ly_fl_'+j)), function(i,v){
            if(v.state !== 0 && v.coords != null && v.coords != ''){
                var index = $.inArray(v.unit, unitCodes);
                if(index >= 0){
                    $('#Map_L'+j).append('<area data-key="'+v.unit+'" alt="'+v.code+'" data-full="'+v.shopState+'" data-area="'+v.area+'" href="/v2/category?id='+v.code+'&type=leasing&storeCode=OLMALL190117000001" shape="poly" coords="'+v.coords+'" />');
                } else {
    ...
    $('.floor_plan').on('click', function(){
        $('map').html('');
        GetMap($(this).attr('id'),'luoyang-sbm','OLMALL190117000001');
    });
...
function getShopsInfo() {
    $.ajax({
        url: $.api.baseNew+"/comm-wechatol/api/shop/base/findAllByStoreCode?storeCode=OLMALL190117000001",
        type: "GET",
        async: false,
        dataType: "json",
...
function renderShopList(s,category){
    ...
    $.each(shop, function(j,w){
        if(w.state == 1 && w.businessFormatChs.indexOf(category) >= 0){
            ...
            GetMapRecommand(w.floor,'luoyang-sbm','OLMALL190117000001',j,w);
        }
    })
    ...
    function GetMapRecommand(fn,lk,mc,j,w){
        ...
        $('#empty_stores').append('<a id="shop_'+w.unitCode+'" href="/v2/category?id='+w.remarkSecond+'&type=leasing&storeCode=OLMALL190117000001">\n\
            ...
            success: function (response, status, xhr) {
                if(response.code === 'C0') {
                    $.each(response.data, function(i,v){
                        if(v.state !== 0 && v.coords != null && v.coords != ''){
                            if(v.unit == unit){
                                $('#Map_J'+jfl).append('<area data-key="'+v.unit+'" alt="'+v.code+'" data-full="'+v.shopState+'" data-area="'+v.area+'" href="/v2/category?id='+v.code+'&type=leasing&storeCode=OLMALL190117000001" shape="poly" coords="'+v.coords+'" />');
                            } else {
...
function GetMap(fn,lk,mc){
    ...
    switch (fn) {
        ...
        case '6F':
            fc = '6';
            FN = '六楼';
            vr = 'https://720yun.com/t/29vksq790ie?scene_id=53511784';
            break;
        case '5F':
            fc = '5';
            FN = '五楼';
            vr = 'https://720yun.com/t/29vksq790ie?scene_id=53511782';
            break;
        case '4F':
            fc = '4';
            FN = '四楼';
            vr = 'https://720yun.com/t/29vksq790ie?scene_id=53511780';
            break;
        case '3F':
            fc = '3';
            FN = '三楼';
            vr = 'https://720yun.com/t/29vksq790ie?scene_id=53502948';
            break;
        case '2F':
            fc = '2';
            FN = '二楼';
            vr = 'https://720yun.com/t/29vksq790ie?scene_id=53502945';
            break;
        case '1F':
            fc = '1';
            FN = '一楼';
            vr = 'https://720yun.com/t/29vksq790ie?scene_id=53502942';
            break;
        case 'B1F':
            fc = '0';
            FN = '负一楼';
            vr = 'https://720yun.com/t/29vksq790ie?scene_id=53502951';
            break;
        default:
            fc = '1';
            FN = '一楼';
            ...
            if(response.code === 'C0') {
                $.each(response.data, function(i,v){
                    if(v.state !== 0 && v.coords != null && v.coords != ''){
                        var index = $.inArray(v.unit, unitCodes);
                        if(index >= 0){
                            $('#Map_S'+fl).append('<area data-key="'+v.unit+'" alt="'+v.code+'" data-full="'+v.shopState+'" data-area="'+v.area+'" href="/v2/category?id='+v.code+'&type=leasing&storeCode=OLMALL190117000001" shape="poly" coords="'+v.coords+'" />');
                        } else {
...
function getCoordsByFloor(fl) {
    ...
    $.ajax({
        url: $.api.baseNew+"/onlineleasing-customer/api/base/coords/OLMALL190117000001/"+floorDesc+"",
        type: "GET",
        async: false,
        beforeSend: function(request) {
            request.setRequestHeader("Lang", $.cookie('lang'));
        ...
        success: function (response, status, xhr) {
            if(response.code === 'C0') {
                sessionStorage.setItem('ly_fl_'+fl, JSON.stringify(response.data));
                $.each(response.data, function(i,v){
                    if(v.state !== 0 && v.coords != null && v.coords != ''){
                        var index = $.inArray(v.unit, unitCodes);
                        if(index >= 0){
                            $('#Map_L'+fl).append('<area data-key="'+v.unit+'" alt="'+v.code+'" data-full="'+v.shopState+'" data-area="'+v.area+'" href="/v2/category?id='+v.code+'&type=leasing&storeCode=OLMALL190117000001" shape="poly" coords="'+v.coords+'" />');
                        } else {
...
function drawRecommandShopsByFloor(map,level,jfl){
    ...
    $('#J'+jfl).show();
    $('#J'+jfl).find('img').attr('src','/views/assets/base/img/content/floor-plan/luoyang-sbm/'+level+'F.png');
