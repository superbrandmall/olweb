...
$(document).ready(function(){
    ...
    for(var j=0;j<=3;j++){
      if(!sessionStorage.getItem('bs_fl_'+j) || sessionStorage.getItem('bs_fl_'+j) == null || sessionStorage.getItem('bs_fl_'+j) == ''){
        getCoordsByFloor(j);
      } else {
        $.each($.parseJSON(sessionStorage.getItem('bs_fl_'+j)), function(i,v){
            if(v.state !== 0 && v.coords != null && v.coords != ''){
                var index = $.inArray(v.unit, unitCodes);
                if(index >= 0){
                    $('#Map_L'+j).append('<area data-key="'+v.unit+'" alt="'+v.code+'" data-full="'+v.shopState+'" data-area="'+v.area+'" href="/v2/category?id='+v.code+'&type=leasing&storeCode=OLMALL180917000002" shape="poly" coords="'+v.coords+'" />');
                } else {
...
function renderShopList(s,category){
    ...
    $.each(shop, function(j,w){
        if(w.state == 1 && w.businessFormatChs.indexOf(category) >= 0){
            ...
            GetMapRecommand(w.floor,'baoshan-tm','OLMALL180917000002',j,w);
        }
            ...
            success: function (response, status, xhr) {
                if(response.code === 'C0') {
                    $.each(response.data, function(i,v){
                        if(v.state !== 0 && v.coords != null && v.coords != ''){
                            if(v.unit == unit){
                                $('#Map_J'+jfl).append('<area data-key="'+v.unit+'" alt="'+v.code+'" data-full="'+v.shopState+'" data-area="'+v.area+'" href="/v2/category?id='+v.code+'&type=leasing&storeCode=OLMALL180917000002" shape="poly" coords="'+v.coords+'" />');
                            } else {
            ...
            if(response.code === 'C0') {
                $.each(response.data, function(i,v){
                    if(v.state !== 0 && v.coords != null && v.coords != ''){
                        var index = $.inArray(v.unit, unitCodes);
                        if(index >= 0){
                            $('#Map_S'+fl).append('<area data-key="'+v.unit+'" alt="'+v.code+'" data-full="'+v.shopState+'" data-area="'+v.area+'" href="/v2/category?id='+v.code+'&type=leasing&storeCode=OLMALL180917000002" shape="poly" coords="'+v.coords+'" />');
                        } else {
...
function getCoordsByFloor(fl) {
    ...
    $.ajax({
        url: $.api.baseNew+"/onlineleasing-customer/api/base/coords/OLMALL180917000002/"+floorDesc+"",
        type: "GET",
        async: false,
        beforeSend: function(request) {
            request.setRequestHeader("Lang", $.cookie('lang'));
        ...
        success: function (response, status, xhr) {
            if(response.code === 'C0') {
                sessionStorage.setItem('bs_fl_'+fl, JSON.stringify(response.data));
                $.each(response.data, function(i,v){
                    if(v.state !== 0 && v.coords != null && v.coords != ''){
                        var index = $.inArray(v.unit, unitCodes);
                        if(index >= 0){
                            $('#Map_L'+fl).append('<area data-key="'+v.unit+'" alt="'+v.code+'" data-full="'+v.shopState+'" data-area="'+v.area+'" href="/v2/category?id='+v.code+'&type=leasing&storeCode=OLMALL180917000002" shape="poly" coords="'+v.coords+'" />');
                        } else {
...
function drawRecommandShopsByFloor(map,level,jfl){
    ...
    $('#J'+jfl).show();
    $('#J'+jfl).find('img').attr('src','/views/assets/base/img/content/floor-plan/baoshan-tm/'+level+'F.png');
