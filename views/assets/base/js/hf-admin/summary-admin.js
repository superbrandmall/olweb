...
$(document).ready(function(){
    ...
    /*var space = [2600,2825];
    for(var i=0;i<2;i++){
        $('#summary_area_percentage tbody').append('<tr><td>'+floorDesc[i]+'</td><td>'+0+'%</td><td">'+0+'%</td><td>'+0+'%</td><td>'+100+'%</td></tr>');
        $('#summary_area tbody').append('<tr><td>'+floorDesc[i]+'</td><td>'+0+'m<sup>2</sup></td><td>'+0+'m<sup>2</sup></td><td>'+0+'m<sup>2</sup></td><td>'+space[i]+'m<sup>2</sup></td><td>'+space[i]+'m<sup>2</sup></td></tr>');
        stores_3_total = Math.round(stores_3_total + space[i]);
        stores_total = Math.round(stores_total + space[i]);
        
        $('#summary_units_percentage tbody').append('<tr><td>'+floorDesc[i]+'</td><td>'+0+'%</td><td>'+0+'%</td><td>'+0+'%</td><td>'+100+'%</td></tr>');
        $('#summary_units tbody').append('<tr><td>'+floorDesc[i]+'</td><td>'+0+'个</td><td>'+0+'个</td><td>'+0+'个</td><td>1个</td><td>1个</td></tr>');
...
function getFloorsRentalInfo(fl) {
    $.ajax({
        url: $.api.baseNew+"/onlineleasing-customer/api/base/coords/OLMALL190409000001/"+fl+"",
        type: "GET",
        async: false,
        dataType: "json",
