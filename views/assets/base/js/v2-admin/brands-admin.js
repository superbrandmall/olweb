...
function ShowBrands(p,c){
    ...
    var map = {};
    $.ajax({
        url: $.api.baseNew+"/onlineleasing-admin/api/brand/findAll?size="+c+"&page="+pg+"&sort=code,desc",
        type: "POST",
            ...
            if(response.code === 'C0') {
                ...
                if(response.data.content.length > 0) { 
                    ...
                    $.each(response.data.content, function(i,v){
                        ...
                        if(v.modality2 !== '' && v.modality2 !== null){
                            $.each($.parseJSON(sessionStorage.getItem("modalities")), function(i,u) {
                                $.each(u.children, function(j,w) {
                                    $.each(w.children, function(k,x) {
                                        if(x.code == v.modality2) {
                                            modality2 = x.name || '-';
                                            return false;
                                        }
                                    });
                        ...
                        if(v.modality3 !== '' && v.modality3 !== null){
                            $.each($.parseJSON(sessionStorage.getItem("modalities")), function(i,u) {
                                $.each(u.children, function(j,w) {
                                    $.each(w.children, function(k,x) {
