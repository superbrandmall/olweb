        ...
        success: function (response, status, xhr) {
            ...
            if(response.code === 'C0') {
                ...
                if(response.data != '' && response.data != null){
                    ...
                    $('#budgetDayRent').text(accounting.formatNumber(data.budgetDayRent) || '/');
            ...
            <td>'+(v.paymentDate || '-')+'</td></tr>')
                    ...
                    if(data.processApproveList.length > 0) {
                        ...
                        $.each(data.processApproveList, function(i,v) {
                            switch (v.activityCode) {
                                case "2":
                                    $('#financeApprove').addClass('active').find('.approveName').text(v.approveName);
                                    break;
                                case "3":
                                    $('#bizApprove').addClass('active').find('.approveName').text(v.approveName);
                                    break;
                                case "4":
                                    $('#legalApprove').addClass('active').find('.approveName').text(v.approveName);
                                    break;
                                case "5":
                                ...
                                case "7":
                                    ...
                                    break;
                                case "10":
                                    $('#leasingPresident').addClass('active').find('.approveName').text(v.approveName);
                                    break;
                                default:
                                    break;
                            }
...
function approveFlowInput(txt) {
    ...
    $.ajax({
        ...
        error: function(jqXHR, textStatus, errorThrown) {
            ...
            window.location.href = "/id/"+getURLBizId().toLowerCase()+"/lotus-approval-opinion?s=fail";
        }
    });
}