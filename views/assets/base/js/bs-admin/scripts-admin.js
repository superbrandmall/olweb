                ...
                $.each(response.data, function(i,v){
                    if((v.subType == '正柜' || v.subType == 'THEAT') && v.state != 0 ){
                        ...
                        /*if(v.brandName != null && v.brandName != '' && v.coords != null && v.coords != '' && (v.shopState == 0 || v.shopState == 2)) {
                            if(itm % 2 == 0){
                                $('#shops_info tbody').append('<tr><td><a href="/ly-admin/?f='+(getURLParameter('f') || '1')+'&id='+v.code+'">'+v.brandName+'</a></td></tr>');
                            } else {
                                $('#shops_info tbody').find('tr').last().append('<td><a href="/ly-admin/?f='+(getURLParameter('f') || '1')+'&id='+v.code+'">'+v.brandName+'</a></td>');
                            }
                            itm++;
                        }*/
                    }
        ...
        success: function (response, status, xhr) {
            ...
            if(response.code === 'C0') {
                ...
                if(shop.brandName != null && shop.brandName != '') {
                    if(shop.shopState == 1) {
                        showShopSales(shop.brandName,'201001A'+shop.unit,contractExpireYear+((contractExpireMonth<10 ? '0' : '') + contractExpireMonth+((contractExpireDate<10 ? '0' : '') + contractExpireDate)));
                    } else {
                        showShopSales(shop.brandName,'201001A'+shop.unit,DecrDates(date,2));
                    }
                } else {
                    $('#weeks').text('4');
                }
