...
function modifyTypeCheck() {
    if($('#contractModifyType').val() != ''){
        ...
        $('#modifyEffectTime').parent().parent().parent().parent().show(); //恢复显示"变更生效日"
        $('#rewardDate').parent().parent().parent().find('label').html('变更签署日 <span class="btn-box-tool-lg">*</span>'); //恢复页面上文字"变更签署日"
        ...
        switch ($('#contractModifyType').val()) {
            ...
            case "CLAUSE_CHANGE":
                ...
                $('#modifyEffectTime').datepicker('update', $('#startDate').val()); //变更生效日强制与合同开始日相同
                $('#modifyEffectTime').parent().parent().parent().parent().hide(); //临时隐藏"变更生效日"
                $('#rewardDate').parent().parent().parent().find('label').html('变更生效日 <span class="btn-box-tool-lg">*</span>'); //临时将页面上"变更签署日"文字调整成变更生效日
                break;
            default:
                break;
        }
