                ...
                if(response.data != '' && response.data != null && response.data.formType == 'modify'){
                    ...
                    if($.request.content != '' && $.request.content != null){
                        ...
                        if(data.oldContractInfo.fixedRentList.length > 0) {
                            ...
                            } else if(data.oldContractInfo.salesList != null && data.oldContractInfo.salesList.length > 0) {                        
                        ...
                        if(data.oldContractInfo.propertyFeeYearList != null && data.oldContractInfo.propertyFeeYearList.length > 0) {
                            if(data.propertyFeeYearList != null && data.propertyFeeYearList.length > 0) {
                                ...
                                $.each(data.oldContractInfo.propertyFeeYearList, function(i,v) {
                                    var newDate = ''; 
                                    if(data.propertyFeeYearList != null && data.propertyFeeYearList.length > 0) {
                                        newDate = data.propertyFeeYearList[0].startDate;
                                        if(dateCompare(v.endDate, newDate) != 'smaller'){
                                            v.endDate = DecrDate(newDate);
                                        }
                                    }
                                    ...
                                    if(newDate == '' || dateCompare(v.startDate, newDate) == 'smaller'){
                                        updateRowInvestmentContractAccounttermPropertyFeeYear(JSON.stringify(v),' past');
...
function updateRowInvestmentContractAccounttermPropertyFeeYear(v,p) {
    ...
    var select = document.createElement("select"); //税率
    select.setAttribute("class","select2 taxVat newVAT propertyFeeYearVATDropDown newFee"+p);
    ...
    Ewin.confirm({ message: msg }).on(function (e) {
        ...
        switch (modifyType) {
            case "TENANT_CHANGE":
                ...
                tenantNo = $('#selectTenant').find('option:selected').text().split(' | ')[0];
                break;
            case "BRAND_CHANGE":
